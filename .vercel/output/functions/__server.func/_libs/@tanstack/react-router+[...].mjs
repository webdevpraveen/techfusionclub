  ...
  constructor(options, getStoreConfig) {
    ...
    this.resolveRedirect = (redirect) => {
      ...
      if (!redirect.options.href || redirect.options._builtLocation) {
        const href =
          (redirect.options._builtLocation ?? this.buildLocation(redirect.options)).publicHref ||
          "/";
...
function createLoaderTask$1(router, lane, index, tasks, semanticParent, options) {
  ...
  const outcome = (
    ...
    if (blocking) {
      ...
      if (result[0] === SUCCESS$1) {
        if (preload && routeLoader && !options[0].signal.aborted)
      ...
      await waitFor$1(
        outcome
          ? Promise.resolve().then(() =>
              loadRouteChunk(
                getRoute$1(router, match),
                kind === ERROR$1 ? "errorComponent" : "notFoundComponent",
              ),
            )
          : Promise.all([
              loadRouteChunk(getRoute$1(router, match)),
              loadRouteChunk(getRoute$1(router, match), "notFoundComponent"),
            ]),
        controller.signal,
      );
    } catch (cause) {
...
var CatchBoundaryImpl = class extends import_react.Component {
  ...
  render() {
    ...
    return error
      ? import_react.createElement(this.props.errorComponent ?? ErrorComponent, {
          error,
          reset: this.reset,
        })
      : this.props.children;
...
export {
  ...
  _getRenderedMatches as D,
  resolveManifestCssLink as E,
  rootRouteId as F,
  isNotFound as I,
  dehydrateSsrMatchId as M,
  isRedirect as N,
  executeRewriteInput as O,
  isResolvedRedirect as P,
  ...
  RouterProvider as d,
  createRouter as f,
  createRootRouteWithContext as g,
  createFileRoute as h,
  disposeSsrResponseDetached as i,
  decodePath as j,
  invariant as k,
  Scripts as l,
  lazyRouteComponent as m,
